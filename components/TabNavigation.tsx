...
export function TabNavigation({ 
  ...
  return (
    <div className={`relative shrink-0 w-full ${className}`}>
      <div ref={scrollRef} className="box-border content-stretch flex flex-row items-center justify-start overflow-hidden p-0 relative w-full">
        {tabs.map((tab) => {
          ...
          return (
            <div 
              key={tab}
              className="box-border content-stretch flex flex-row gap-[10px] items-center justify-center px-[12px] py-[9px] relative shrink-0 cursor-pointer"
      ...
      {canScrollLeft && (
        <div className="pointer-events-none absolute left-0 top-0 bottom-0 w-4 bg-white" />
      )}
      {canScrollRight && (
      ...
      {canScrollLeft && (
        <button
          type="button"
          aria-label="이전 탭"
          className="absolute left-1 top-1/2 -translate-y-1/2 bg-white shadow-sm rounded-full size-8 flex items-center justify-center cursor-pointer"
      ...
      {canScrollRight && (
        <button
          type="button"
          aria-label="다음 탭"
          className="absolute right-1 top-1/2 -translate-y-1/2 bg-white shadow-sm rounded-full size-8 flex items-center justify-center cursor-pointer"
      ...
      <div
        aria-hidden="true"
        className="absolute border-[#f2f4f6] border-[0px_0px_1px] border-solid inset-0 pointer-events-none"
      />
    </div>
  );
}